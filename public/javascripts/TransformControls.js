...
( function () {
	...
	THREE.TransformGizmoScale = function () {
		...
		this.handleGizmos = {
			...
			X: [
				[ new THREE.Mesh( arrowGeometry, new GizmoMaterial( { color: 0xff0000 } ) ), [ 5, 0, 0 ], [ 0, 0, - Math.PI / 2 ] ],
				[ new THREE.Line( lineXGeometry, new GizmoLineMaterial( { color: 0xff0000 } ) ) ]
			],

			Y: [
				[ new THREE.Mesh( arrowGeometry, new GizmoMaterial( { color: 0x00ff00 } ) ), [ 0, 5, 0 ] ],
				[ new THREE.Line( lineYGeometry, new GizmoLineMaterial( { color: 0x00ff00 } ) ) ]
			],

			Z: [
				[ new THREE.Mesh( arrowGeometry, new GizmoMaterial( { color: 0x0000ff } ) ), [ 0, 0, 5 ], [ Math.PI / 2, 0, 0 ] ],
				[ new THREE.Line( lineZGeometry, new GizmoLineMaterial( { color: 0x0000ff } ) ) ]
			],

			XYZ: [
				[ new THREE.Mesh( new THREE.BoxBufferGeometry( 1.25, 1.25, 1.25 ), new GizmoMaterial( { color: 0xffffff, opacity: 2.5 } ) ) ]
			]
		...
		this.pickerGizmos = {
			...
			X: [
				[ new THREE.Mesh( new THREE.CylinderBufferGeometry( 2, 0, 1, 4, 1, false ), pickerMaterial ), [ 6, 0, 0 ], [ 0, 0, - Math.PI / 2 ] ]
			],

			Y: [
				[ new THREE.Mesh( new THREE.CylinderBufferGeometry( 2, 0, 1, 4, 1, false ), pickerMaterial ), [ 0, 6, 0 ] ]
			],

			Z: [
				[ new THREE.Mesh( new THREE.CylinderBufferGeometry( 2, 0, 1, 4, 1, false ), pickerMaterial ), [ 0, 0, 6 ], [ Math.PI / 2, 0, 0 ] ]
			],

			XYZ: [
				[ new THREE.Mesh( new THREE.BoxBufferGeometry( 4, 4, 4 ), pickerMaterial ) ]
			]
	...
	THREE.TransformControls = function ( camera, domElement ) {
		...
		this.attach = function ( object ) {
			...
			this.object = object;
			this.visible = true;
			this.annCubic.position.copy(this.object.position);


			var id = (this.object.position.z)/10 +1;

			updateObjectMaterial(this.annCubic,id);
			...
			if ( _mode === "translate" ) {
				...
				if ( scope.space === "world" || scope.axis.search( "XYZ" ) !== - 1 ) {
					...
					if(event.altKey || gui_config.move){
						...
						if(oldMove == null) {
							oldMove = (new THREE.Vector3()).copy(point);
							// scope.annCubic.scale = new THREE.Vector3();
						}else{
							// var move = THREE.Vector3.subVectors(point,oldMove);
							var move = new THREE.Vector3();
							move.copy(point);
							move.sub(oldMove);
							...
							if(point.z != oldMove.z ){


								// var id = Math.floor(scope.annCubic.scale.z/10)+1;
                                //
								// var sId = ("00"+id).slice(-2);
								// scope.annCubic.material.materials[4].map = THREE.ImageUtils.loadTexture( "output/ser002img000"+sId+".dcm.jpeg" );
								// scope.annCubic.material.needsUpdate = true;

								var id = (scope.annCubic.position.z + Math.floor(scope.annCubic.scale.z/2))/10 +1;
		...
		function updateObject(object){
			...
			var textureW = 512;
			var textureH = 512;
			var cubicW = object.scale.x;
			var cubicH = object.scale.y;
			var offsetW = 256+(object.position.x - cubicW/2);
			var offsetH = 256+(object.position.y - cubicH/2);
